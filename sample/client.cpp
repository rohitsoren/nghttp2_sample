...
void startClient()
{
	...
	if( sock_fd <= 0)
	{
		printf("ERROR! startClient->socket failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
		return ;
	}
	...
	if(l_ret <= 0)
	{
		printf("ERROR! startClient->inet_pton failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
		close(sock_fd);
		return ;
	}
	address.sin_port = ;
	...
	if(l_ret < 0)
	{
		printf("ERROR! startClient->bind failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
		close(sock_fd);
		return ;
	}
	...
	if(l_ret < 0)
	{
		printf("ERROR! startClient->listen failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
		close(sock_fd);
		return ;
	}
	...
	while(1)
	{
		...
		if(client_conn <= 0)
		{
			printf("ERROR! startClient->accept failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
			continue ;
		}
		printf("startClient connection = <%d> received\n", client_conn);

		l_ret = pthread_attr_init(&attr);
		if(l_ret != 0)
		{
			printf("ERROR! startClient->pthread_attr_init failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
			close(client_conn);
			continue ;
		}
		...
		if(l_ret != 0)
		{
			printf("ERROR! startClient->pthread_attr_setdetachstate failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
			close(client_conn);
			continue ;
		}
		...
		if(l_ret != 0)
		{
			printf("ERROR! startClient->pthread_create failed errno=<%d> sock_fd=<%d>\n", errno, sock_fd);
			close(client_conn);
			continue ;
		}
	}
